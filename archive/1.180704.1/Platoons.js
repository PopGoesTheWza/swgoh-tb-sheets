...
function ResetPlatoon(phase, zone, platoonRow, rows, show) {
  ...
  for (var platoon = 0; platoon < MaxPlatoons; ++platoon) {
    // clear the contents
    var range = sheet.getRange(platoonRow, 4 + (platoon * 4), MaxPlatoonHeroes, 1);
    range.setValue(null);
...
function PlayerAvailable(player, unavailable) {
  ...
  return unavailable.some(function(e) { return e[0].length > -1 && player == e[0]; });
}
...
function CreateDropdown(playerList, range) {
  ...
  var rule = SpreadsheetApp.newDataValidation().requireValueInList(formatList).build();
...
function RecommendPlatoons() {
  ...
  //var maxGear = get_minimum_gear_level_();
  ...
  var heroData = heroesSheet.getRange(1, 1, 1 + heroCount, HeroPlayerColOffset + MaxPlayers).getValues();
  var shipData = shipsSheet.getRange(1, 1, 1 + shipCount, HeroPlayerColOffset + MaxPlayers).getValues();
  ...
  for (var o = 0, oLen = platoonOrder.length; o < oLen; ++o) {
    ...
    var range = sheet.getRange(cur.row, baseCol + 1 + platoonOffset, MaxPlatoonHeroes, 1);
    range.clearContent();
    range.clearDataValidations();
    range.setFontColor("Black");
    ...
    var skip = sheet.getRange(cur.row + 15, baseCol + platoonOffset + 1, 1, 1).getValue() === "SKIP";
    if (skip) {
      cur.possible = false;
    }

    // cycle through the units
    var units = sheet.getRange(cur.row, baseCol + platoonOffset, MaxPlatoonHeroes, 1).getValues();
    for (var h = 0; h < MaxPlatoonHeroes; ++h) {
      var unitName = units[h][0];
      var idx = PlatoonMatrix.length;

      var playersRange = sheet.getRange(cur.row + h, baseCol + 1 + platoonOffset);
      if (unitName.length == 0) {
      ...
      var rec = GetRecommendedPlayers(unitName, phase, (cur.isGround) ? heroData : shipData, cur.isGround, unavailable);
  ...
  for (var o = 0, oLen = platoonOrder.length; o < oLen; ++o) {
    ...
    if (cur.possible == false) {
      ...
      var platoonRange = sheet.getRange(cur.row, baseCol + 1 + plattonOffset, MaxPlatoonHeroes, 1);
  ...
  for (var o = 0, oLen = platoonOrder.length; o < oLen; ++o) {
    ...
    for (var h = 0; h < MaxPlatoonHeroes; ++h) {
      var plattonOffset = cur.num * 4;
      var platoonRange = sheet.getRange(cur.row + h, baseCol + 1 + plattonOffset);

      var defaultValue = "";
      for (var playerIdx = 0, playerLen = PlatoonMatrix[matrixIdx].players.length; playerIdx < playerLen; ++playerIdx) {
        ...
        if (cur.isGround) {
          ...
          for (var u = 1, uLen = UsedHeroes[heroRow].length; u < uLen; ++u) {
            ...
            var playerAvail = (placementCount[cur.zone][playerName] == null || placementCount[cur.zone][playerName] < maxPlayerDonations);
            if (playerAvail && UsedHeroes[0][u] == playerName && UsedHeroes[heroRow][u] == false) {
          ...
          for (var u = 1, uLen = UsedShips[heroRow].length; u < uLen; ++u) {
            var playerName = PlatoonMatrix[matrixIdx].players[playerIdx];
            var playerAvail = (placementCount[cur.zone][playerName] == null || placementCount[cur.zone][playerName] < maxPlayerDonations);
            if (playerAvail && UsedShips[0][u] == playerName && UsedShips[heroRow][u] == false) {
      ...
      } else if (defaultValue.length > 0 && (PlatoonMatrix[matrixIdx].count + 3) > PlatoonMatrix[matrixIdx].pCount) {
