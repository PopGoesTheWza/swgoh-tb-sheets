...
function GetPhaseHours() {
  var sheet = SpreadsheetApp.getActive().getSheetByName("Discord");
  var value = sheet
    .getRange(WebhookPhaseHoursRow, DiscordWebhookCol)
    .getValue();
...
function GetWebhookWarnIntro(phase, mention) {
  var defaultVal =
    "Here are the __Rare Units__ to watch out for in __Phase " +
    phase +
    "__. **Check with an officer before donating to Platoons/Squadrons that require them.**";

  return (
    "\n\n" +
    GetWebhookTemplate(phase, WebhookWarnRow, defaultVal) +
    " " +
    mention
  );
}
...
function GetWebhookRareIntro(phase, mention) {
  var defaultVal =
    "Here are the Safe Platoons and the Rare Platoon donations for __Phase " +
    phase +
    "__. **Do not donate heroes to the other Platoons.**";

  return (
    "\n\n" +
    GetWebhookTemplate(phase, WebhookRareRow, defaultVal) +
    " " +
    mention
  );
}
...
function GetWebhookDepthIntro(phase, mention) {
  var defaultVal =
    "Here are the Platoon assignments for __Phase " +
    phase +
    "__. **Do not donate heroes to the other Platoons.**";

  return (
    "\n\n" +
    GetWebhookTemplate(phase, WebhookDepthRow, defaultVal) +
    " " +
    mention
  );
}
...
function GetWebhookDesc(phase) {
  ...
  var sheet = SpreadsheetApp.getActive().getSheetByName("Discord");
  var text = sheet
    .getRange(WebhookDescRow + phase - 1, DiscordWebhookCol + (isLight ? 0 : 1))
    .getValue();
...
function GetPlayerMentions() {
  ...
  for (var i = 0, iLen = data.length; i < iLen; ++i) {
    ...
    if (name != null && name.length > 0 && result[name] == null) {
      ...
      result[name] =
        data[i][1] == null || data[i][1].length == 0 ? name : data[i][1];
    }
  }
  ...
  for (var z = 0; z < MaxPlatoonZones; ++z) {
    ...
    for (var p = 0; p < MaxPlatoons; ++p) {
      var platoonData = sheet
        .getRange(platoonRow, 4 + p * 4, MaxPlatoonHeroes, 2)
        .getValues();
...
function GetPlatoonDonations(platoon, donations, rules, playerMentions) {
  ...
  for (var h = 0; h < MaxPlatoonHeroes; ++h) {
    ...
    for (d = 0, dLen = result.length; d < dLen; ++d) {
  ...
  for (var z = 0; z < MaxPlatoonZones; ++z) {
    ...
    for (var p = 0; p < MaxPlatoons; ++p) {
      var platoonData = sheet
        ...
        .getValues();
      var rules = sheet
        .getRange(platoonRow, 5 + p * 4, MaxPlatoonHeroes, 1)
        .getDataValidations();
      var platoon = GetPlatoonDonations(
        platoonData,
        donations,
        rules,
        playerMentions
      ...
      if (platoon != null) {
        var platoonName = "#" + (p + 1);
        validPlatoons +=
          validPlatoons.length > 0 ? ", " + platoonName : platoonName;
  ...
  if (byType == "Player") {
    ...
    for (var d = 0, dLen = donations.length; d < dLen; ++d) {
      ...
      for (var n = 0, nLen = names.length; n < nLen; ++n) {
        ...
        var foundName = false;
        var pLen;
        for (p = 0, pLen = playerDonations.length; p < pLen; ++p) {
          if (playerDonations[p][0] == name) {
            if (
              d >= groundStart &&
              playerDonations[p][1].indexOf(heroLabel) < 0
            ) {
              playerDonations[p][1] += "\n" + heroLabel + unit;
            } else {
        ...
        if (!foundName) {
          playerDonations[playerDonations.length] = [
            name,
            d >= groundStart ? heroLabel + unit : shipLabel + unit
          ];
        }
      }
    }
...
function RegisterWebhookTimer() {
  ...
  if (startTime && phaseHours) {
    ...
    ScriptApp.getProjectTriggers()
      .filter(function(e) {
        return e.getHandlerFunction() === "SendTimedWebhook";
      })
      .forEach(function(e) {
