...
function PopulateTBTable(data, members, heroes) {
  const roster = SpreadsheetApp.getActive().getSheetByName("Roster").getRange(2, 2, get_guild_size_(), 1).getValues() as string[][]
  const hIdx = []
  ...
  for (let c = 0; c < roster.length; c++) {
    ...
    for (let r = 0; r < data.length; r++) {
      ...
      table[r + 1][c] =  requirementsMet ? `${o.rarity}*` : `${o.rarity}*L${o.level}G${o.gear_level}`
    }
  }
  return table
}
...
function UpdateGuildRoster(members) {
  ...
  const addMembers = sheet.getRange(2, MetaAddPlayersCol, MaxPlayers, 2).getValues()
  const removeMembers = sheet.getRange(2, MetaRemPlayersCol, MaxPlayers, 1).getValues()
  ...
  const sortFunction = get_sort_roster_()
    ? (a, b) => a.name.toLowerCase().localeCompare(b.name.toLowerCase()) // sort roster by player name
    : (a, b) => b.gp - a.gp  // sort roster by GP
  ...
  const result = members.map( e => [[e.name], [e.link], [e.gp], [e.heroes_gp], [e.ships_g]] )
...
*/
  ...
  //This will update Roster Sheet with names and GPs, will also return a new members array with added/deleted from sheet
...
*/
  var range = tbSheet.getRange(2, 3, lastHeroRow, 6).getValues();
  table = PopulateTBTable(range, members, heroes);

  // store the table of player data
  //  tbSheet.getRange(1, HeroPlayerColOffset + 1, table.length, table[0].length).setValues(table);
  var width = table.reduce(function(a, e) {
    return Math.max(a, e.length);
  }, 0);
  table = table.map(function(e) {
    var result = e;
    if (e.length !== width) {
      result = e.concat(Array(width).fill(null)).slice(0, width);
    }
    return result;
  });
  tbSheet
    .getRange(1, MetaTBColOffset, table.length, table[0].length)
    .setValues(table);
}
