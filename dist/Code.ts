...
var MetaUndergearRow = 2;
var MetaUnitPerPlayerRow = 11;

var MetaMinLevelRow = 5;
var MetaUnitPowerRow = 8;
...
//var ShipPlayerColOffset = 9;

// Roster tab columns
//var RosterShipCountCol = 10;

var MetaDataSourceCol = 4;
var MetaDataSourceRow = 14;

var MetaUnitCountsCol = 5;
var MetaHeroesCountRow = 5;
var MetaShipsCountRow = 8;

var MetaAddPlayersCol = 16;
var MetaRemPlayersCol = 18;

// Hero/Ship tab columns
var HeroPlayerColOffset = 11;
var ShipPlayerColOffset = 11;

// Roster Size info
var MetaGuildSizeRow = 5;
var MetaGuildSizeCol = 12;

var MetaTBColOffset = 10;
...
function FullClear() {
  ...
  sheet.getRange(2, 2, MaxPlayers, 9).clearContent();
  sheet = SpreadsheetApp.getActive().getSheetByName("TB");
  sheet.getRange(1, MetaTBColOffset, 50, MaxPlayers).clearContent();
  sheet.getRange(2, 1, 50, MetaTBColOffset - 1).clearContent();
  ResetPlatoons();
  sheet = SpreadsheetApp.getActive().getSheetByName("Heroes");
  sheet.getRange(1, 1, 300, MaxPlayers + HeroPlayerColOffset).clearContent();
  sheet = SpreadsheetApp.getActive().getSheetByName("Ships");
  sheet.getRange(1, 1, 300, MaxPlayers + ShipPlayerColOffset).clearContent();
}
...
function get_tag_filter_() {
  ...
  var value = sheet.getRange(MetaFilterRow, MetaFilterCol).getValue();

  return value;
}

function get_character_count_() {
  return Number(SpreadsheetApp.getActive().getSheetByName("Meta").getRange(MetaHeroesCountRow, MetaUnitCountsCol).getValue())
}

function get_ship_count_() {
  return Number(SpreadsheetApp.getActive().getSheetByName("Meta").getRange(MetaShipsCountRow, MetaUnitCountsCol).getValue())
}

function get_character_tag_() {
  // get the tag
  var sheet = SpreadsheetApp.getActive().getSheetByName("Meta");
  var value = sheet.getRange(MetaTagRow, MetaTagCol).getValue();
...
function get_minimum_gear_level_() {
  ...
  var sheet = SpreadsheetApp.getActive().getSheetByName("Meta");
  var value = sheet.getRange(MetaUndergearRow, MetaUndergearCol).getValue();
...
function get_minimum_character_gp_() {
  var sheet = SpreadsheetApp.getActive().getSheetByName("Meta");
  var value = sheet.getRange(MetaUnitPowerRow, MetaUndergearCol).getValue();
...
function get_maximum_platoon_donation_() {
  var sheet = SpreadsheetApp.getActive().getSheetByName("Meta");
  var value = sheet.getRange(MetaUnitPerPlayerRow, MetaUndergearCol).getValue();
...
function get_data_source_() {
  var sheet = SpreadsheetApp.getActive().getSheetByName("Meta");
  var value = sheet.getRange(MetaDataSourceRow, MetaDataSourceCol).getValue();

  return value;
}

function get_guild_size_() {
  var sheet = SpreadsheetApp.getActive().getSheetByName("Roster");
  return Number(sheet.getRange(MetaGuildSizeRow, MetaGuildSizeCol).getValue());
}
...
function ForceHttps(url) {
  var result = url.replace("http:", "https:");
...
function remove_members_(members, removeMembers) {
  ...
  members.forEach(function(m) {
    if (!should_remove(m, removeMembers)) {
      result.push(m);
    }
  });
  return result;
...
function add_missing_members_(result, addMembers) {
  ...
  addMembers
    ...
    .filter(function(e) {
      ...
      var found = result.some(function(l) {
        return l[1] === url;
      });
...
function GetGuildRoster() {
  ...
  // get the filter & tag
  // var PowerTarget = get_minimum_character_gp_();
...
function find_in_list_(name, list) {
  var result = list.findIndex(function(e) {
    return name === e[0];
  });
...
function get_player_link_() {
  ...
  if (playerLink.length === 0) {
    ...
    var members = SpreadsheetApp.getActive()
      .getSheetByName("Roster")
      .getRange(2, 2, MaxPlayers, 2)
      .getValues();

    // get the player's link from the Roster
    var match = members.find(function(e) {
      return e[0] === memberName;
    });
    if (match) {
      playerLink = match[1];
    }
...
function PlayerSnapshot() {
    ...
    var units = text
      ...
      .forEach(function(e) {
        ...
        var gear = Number.parseRoman(
          e.match(/char-portrait-full-gear-level">([^<]*)/)[1]
        );
        var power = Number(
        ...
        if (heroListIdx >= 0) {
          meta[heroListIdx][1] = Utilities.formatString(
            "%s* G%s L%s P%s",
            stars,
            gear,
            level,
  ...
  baseData.push([
    Utilities.formatString("%s 7* P%s+", tag_filter, PowerTarget),
    countFiltered
  ]);
  baseData.push([
    Utilities.formatString("%s 7* P%s+", character_tag, PowerTarget),
    countTagged
  ]);
